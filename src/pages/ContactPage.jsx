...
const ContactPage = () => {
  ...
  async function fetchAIResponse(option, grade, examples) {
    const userMessage = `下記の言葉を${option}という意味合いでどんなときにも使える拡張した言葉を10個つくり、それを改行のある1．２．のような箇条書きにはしないようにしてください。ただし、箇条書きにしない代わり改行を入れてください。〇〇や-〇〇な〇〇-のような当てはめるべきところは当てはめて出力してください。：\n\n${examples}`;
          ...
          <div className="ai-response-container">
            ...
            {isLoading ? (
              ...
              <div 
                ...
                dangerouslySetInnerHTML={{ __html: aiResponse.replace(/^(\d+\.\s*[^<\n]*)$/gm, '<div class="ai-response-item">$1</div>') }}
              />
            )}
          </div>
        )}
